<template>
  ...
  <n-space vertical align="center">
    <n-h4>房间: {{ homeStore.homeId }}</n-h4>
    <InfoDisplay :user="opponentInfo" />
    <n-gradient-text class="notice-font" :size="20" type="info">
      说点什么吧~
    </n-gradient-text>
    <n-space vertical>
      <div class="chat-board" ref="boardContent" id="chatScroller">
        <div
          class="content-item"
...
import { NSpace, NH4, NButton, NInput, NGradientText } from 'naive-ui'
import { onMounted, ref, nextTick } from 'vue'
import { useGobangStore, useUserStore, useRouterStore, useHomeStore } from '../store'
export default {
  name: "HomeView",
  components: {
    NSpace, NH4, NButton, NInput, NGradientText,
    Back, InfoDisplay,
  },
  setup () {
    ...
    const onWheel = () => {
      let $ac_game_chat = $('#chatScroller');
      $ac_game_chat.on("wheel", (e) => {
        //上下滚动
        if (e.originalEvent.wheelDelta > 0) {
          //向上滚动
          $ac_game_chat.scrollTop($ac_game_chat.scrollTop() - 40)
        }
        if (e.originalEvent.wheelDelta < 0) {
          //向下滚动
          $ac_game_chat.scrollTop($ac_game_chat.scrollTop() + 40)
