...
import { NAvatar, NSpace, NIcon, NButton, NModal, NH5, NUl, NLi, NCard, NText, NGrid, NGridItem } from 'naive-ui'
import { ref } from 'vue'
...
export default {
  name: "GameInfo",
  components: {
    NAvatar, NSpace, NIcon, NButton, NModal, NH5, NUl, NLi, NCard, NText, NGrid, NGridItem,
  ...
  setup (props) {
    ...
    let users = ref([]);
    let ranklistItemStyle = ref(null);
    ...
    const getRanklistItemStyle = (index) => {
      ranklistItemStyle.value = "ranklist-item";
      if (index === 0) {
        ranklistItemStyle.value = "ranklist-item-me";
      }
      return ranklistItemStyle.value;
    }
    return {
      GameController, ranklistModal, howPlayModal,
      users, ranklistName,
      getRankList, howToPlay, getRanklistItemStyle
    }
  }
}
</script>
