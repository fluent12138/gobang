<template>
  <div class="oneput-game">
    <Back />
    <GameInfo class="game-info" :gameInfo="gameInfo" />
    <RankListCarousel class="ranklist-carousel" :mode="'oneput'" />
    <OnePutReputation class="oneput-reputation" />
    <n-grid :x-gap="12" :y-gap="20" :cols="2" class="game-level">
      <n-grid-item v-for="oneput in oneputInfos" :key="oneput.id">
        <GameLevel class="game-level-item" :info="oneput" />
      </n-grid-item>
    </n-grid>
...
export default {
  name: 'OnePutGameView',
  components: {
    Back, GameInfo, RankListCarousel, NGrid, NGridItem, NPagination, NSpace,
